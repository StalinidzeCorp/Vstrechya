...
class CollectionItemInline(admin.TabularInline):
    ...
    def get_photo(self, object):
        return mark_safe(f'<img src="{object.image_url}" width=150>') if object.image_url else '-'
...
class CollectionItemAdmin(admin.ModelAdmin):
    fields = ('id', 'collection', 'image_url', 'description', 'get_photo', 'created_at', 'updated_at')
    readonly_fields = ('id', 'get_photo', 'created_at', 'updated_at')

    def get_photo(self, object):
        return mark_safe(f'<img src="{object.image_url}" width=400>') if object.image_url else '-'
